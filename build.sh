...
if [[ ${TASK} == "publish-dev" ]]; then
    # Push dev image
    TAGS="7.3-alpine-v3.9-dev 7.3-alpine-v3.10-dev 7.3-alpine-v3.11-dev 7.4-alpine-v3.9-dev 7.4-alpine-v3.10-dev 7.4-alpine-v3.11-dev"
    for TAG in ${TAGS}; do
        check_or_push $TAG
    done
fi
...
if [[ ${TASK} == "publish-base" ]]; then
    # Push base image
    TAGS="7.3-alpine-v3.9-base 7.3-alpine-v3.10-base 7.3-alpine-v3.11-base 7.4-alpine-v3.9-base 7.4-alpine-v3.10-base 7.4-alpine-v3.11-base"
    for TAG in ${TAGS}; do
        check_or_push $TAG
    done
fi
